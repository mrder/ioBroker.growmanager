...
class GrowManagerAdapter extends utils.Adapter {
    constructor(options = {}) {
        ...
        this.databaseService = new DatabaseService_1.DatabaseService(log, async (id, val) => { await this.setStateAsync(id, { val, ack: true }); }, async (id) => {
    ...
    async onReady() {
        ...
        this.webDashboard.setLifestyleCallbacks(async (groupId) => {
        ...
        for (const actuator of group.actuators) {
            ...
            if (actuator.powerStateId && !this.subscribedStateIds.has(actuator.powerStateId)) {
                ...
                if (currentPower?.val !== null && currentPower?.val !== undefined) {
                    ...
                    if (actState)
                        this.actuatorService.processFeedback(actuator, actState.feedback, currentPower.val);
                }
            }
        ...
        for (const group of this.growConfig.groups) {
            ...
            for (const actuator of group.actuators) {
                ...
                if ((isFeedback || isCommandAck) && state.ack) {
                    ...
                    if (t) {
                        this.clearTimeout(t);
                        this.pendingVerify.delete(actuator.id);
                    }
                }
    ...
    evaluateCustomAlertRules(config, state) {
            ...
            switch (metric) {
                ...
                default: {
                    ...
                    for (const sensorCfg of config.sensors) {
                        if (sensorCfg.type !== metric)
                            continue;
                        const ss = state.sensors.get(sensorCfg.id);
                        if (ss?.valid && typeof ss.processedValue === 'number')
                            return ss.processedValue;
                    }
                    return null;
                }
            }
        ...
        for (const rule of rules) {
            ...
            switch (rule.condition) {
                case 'above':
                    triggered = val > (rule.threshold ?? 0);
                    break;
                case 'below':
                    triggered = val < (rule.threshold ?? 0);
                    break;
                case 'outside':
                    triggered = val < (rule.thresholdMin ?? 0) || val > (rule.thresholdMax ?? 0);
                    break;
                case 'inside':
                    triggered = val >= (rule.thresholdMin ?? 0) && val <= (rule.thresholdMax ?? 0);
                    break;
        ...
        switch (actuatorType) {
            ...
            case 'supplyFan': {
                ...
                if (groupMode === 'humidity' && (actuatorType === 'exhaustFan' || actuatorType === 'supplyFan')) {
                    ...
                    if (excess > 0)
                        return { wantsOn: true, urgency: Math.min(1, excess / 10), reason: `RH ${hum.toFixed(0)}% > Soll ${target}% – Lüftung für Entfeuchtung` };
     ...
     */
    setActuatorStateWithVerify(actuatorConfig, groupId, value, verifyDelaySec = 10) {
