...
class ConfigurationService {
    ...
    validate(config) {
        ...
        if (!config.groups || !Array.isArray(config.groups)) {
            ...
            for (const g of config.groups) {
                if (!g.id)
                    errors.push('Gruppe ohne ID gefunden');
                if (!g.name)
                    errors.push(`Gruppe ${g.id}: Name fehlt`);
                if (ids.has(g.id))
                    errors.push(`Doppelte Gruppen-ID: ${g.id}`);
                ids.add(g.id);
    ...
    validateGroup(group, profiles) {
        ...
        for (const s of group.sensors ?? []) {
            if (!s.stateId)
                errors.push(`${pfx} Sensor ${s.id}: stateId fehlt`);
            if (sensorIds.has(s.id))
                errors.push(`${pfx}: Doppelte Sensor-ID: ${s.id}`);
            sensorIds.add(s.id);
        ...
        for (const a of group.actuators ?? []) {
            if (!a.commandStateId)
                errors.push(`${pfx} Aktor ${a.id}: commandStateId fehlt`);
            if (actuatorIds.has(a.id))
                errors.push(`${pfx}: Doppelte Aktor-ID: ${a.id}`);
